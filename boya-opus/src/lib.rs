...
impl Toc {
    ...
    pub fn bandwidth(&self) -> BandWidth {
        match self.config() {
            c if (0..=3).contains(&c) || (16..=19).contains(&c) => BandWidth::Narrow,
            c if (4..=7).contains(&c) => BandWidth::Medium,
            c if (8..=11).contains(&c) || (20..=23).contains(&c) => BandWidth::Wide,
            c if (12..=13).contains(&c) || (24..=27).contains(&c) => BandWidth::SuperWide,
            c if (14..=15).contains(&c) || (28..=31).contains(&c) => BandWidth::Full,
            _ => unreachable!("OPUS config is always less than 32"),
        }
    }
    ...
    pub fn samples_per_frame(&self, fs: SampleRate) -> usize {
        ...
        match (self.mode(), self.frame_size()) {
            ...
            (Mode::Silk, FrameSize::FS10)
            | (Mode::Silk, FrameSize::FS20)
            | (Mode::Silk, FrameSize::FS40) => (fs << (self.0 >> 3 & 0x3)) / 100,
...
mod test {
    ...
    fn test_toc_samples_per_frame() {
        ...
        assert_eq!(
            spfs.as_slice(),
            &[
                80, 160, 320, 480, 80, 160, 320, 480, 80, 160, 320, 480, 80, 160, 80, 160, 20, 40,
                80, 160, 20, 40, 80, 160, 20, 40, 80, 160, 20, 40, 80, 160
            ]
        );
    }
}
